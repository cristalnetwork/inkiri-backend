...
exports.validateWriteAuth = async(req, res, next) => {
  ...
  if(!ref_account)
  {
    ...
    if(!service_id)
      return res.status(404).send({error:'Business NOT SET #1'});

    let service = null;

    try {
        service = await ServiceModel.findById(service_id);
    } catch (e) {
        return res.status(404).send({error:'Business NOT SET #2'});
    }
    ref_account = service.account_name;
    biz         = service.created_by;
  }
  if(!ref_account)
    return res.status(404).send({error:'Business NOT SET #3'});
